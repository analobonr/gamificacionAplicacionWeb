package constantes;


public class Mensajes {
	
	//Mensajes de error
	public static final String HEADERUPS = "UPS!";
	public static final String HEADERERROR = "¡Error!";
	public static final String ERRORACCESO = "Usuario o contraseña incorrectos.";
	public static final String ERRORNEWPASS = "Se ha producido un error en la autogeneración de la nueva contraseña. Inténtelo de nuevo.";
	public static final String ERRORNEWPASS2 = "Complete la dirección de correo electrónico para recibir su nueva contraseña.";
	public static final String ERRORSUBIRZIP = "Debe subir el fichero zip del juego antes de añadirlo.";
	
	//Mensajes de info
	public static final String HEADERNEWPASS = "Nueva contraseña";
	public static final String NEWPASS = "En su dirección de correo electrónico [MAIL] podrá encontrar su nueva contraseña.";
	public static final String HEADERNEWUSER = "Nuevo usuario";
	public static final String NEWUSER = "Su usuario [MAIL] ha sido registrado correctamente.";
	
	//Mensajes para el Mail
	public static final String ASUNTOMAIL = "ADJA: Nuevo acceso";
	public static final String MENSAJEMAIL = "Buenas, \nSu nueva contraseña para acceder a nuestra plataforma es: [NEWPASS]\nEsperamos que disfrute la experiencia. \nUn saludo.";

}

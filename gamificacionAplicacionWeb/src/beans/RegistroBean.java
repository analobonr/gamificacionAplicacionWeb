...
public class RegistroBean implements Serializable {
	...
	public boolean nuevoUsuario(Profesor p) {
		...
		try {
			ResponseEntity<Object> response = restTemplate.postForEntity(url, p, null);

			// Error
			if (!response.getStatusCode().is2xxSuccessful()) {
				fContext.addMessage(null, new FacesMessage(Mensajes.HEADERERROR, Mensajes.ERRORNEWUSER));
				nuevoUsuario = false;
				...
				fContext.addMessage(null,
						new FacesMessage(Mensajes.HEADERNEWUSER, Mensajes.NEWUSER.replace("[MAIL]", email)));
	...
	public void autogenerarPass(String email) {
		...
		try {
			...
			System.err.println("Error al generar la nueva contraseña: " + envioEx);
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(null, new FacesMessage(Mensajes.HEADERERROR, Mensajes.ERRORNEWPASS));
		}
	}
